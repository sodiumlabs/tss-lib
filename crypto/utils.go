...
func ECBasePoint2(curve elliptic.Curve) (pt *ECPoint, err error) {
	...
	if overflow := xf.SetByteSlice(params.Gx.Bytes()); overflow {
		return nil, errors.New("ECBasePoint2() Gx overflow")
	}

	yf := secp256k1.FieldVal{}
	if overflow := yf.SetByteSlice(params.Gy.Bytes()); overflow {
		return nil, errors.New("ECBasePoint2() Gy overflow")
	}

	G := btcec.NewPublicKey(&xf, &yf)
	bz := G.SerializeCompressed()
	for i := 0; i < minRounds || pt == nil; i++ {
		if i >= 10 {
			err = fmt.Errorf("ECBasePoint2() too many rounds (max: 10) %d : %d", minRounds, i)
			return
		}
