...
func FinalizeGetAndVerifyFinalSig(
...
) (*SignatureData, *btcececdsa.Signature, *tss.Error) {
	if len(otherSIs) == 0 {
	...
	if overflow := rm.SetByteSlice(r.Bytes()); overflow {
		return nil, nil, FinalizeWrapError(fmt.Errorf("r overflow"), ourP)
	}

	sm := secp256k1.ModNScalar{}
	if overflow := sm.SetByteSlice(s.Bytes()); overflow {
		return nil, nil, FinalizeWrapError(fmt.Errorf("s overflow"), ourP)
	}
	...
	if overflow := xf.SetByteSlice(pk.X.Bytes()); overflow {
		return nil, nil, FinalizeWrapError(fmt.Errorf("x overflow"), ourP)
	}
	yf := secp256k1.FieldVal{}
	if overflow := yf.SetByteSlice(pk.Y.Bytes()); overflow {
		return nil, nil, FinalizeWrapError(fmt.Errorf("y overflow"), ourP)
	}

	btcecpk := btcec.NewPublicKey(&xf, &yf)

	if ok = btcecSig.Verify(msg.Bytes(), btcecpk); !ok {
